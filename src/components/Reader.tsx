...
export default function Reader({
  ...
  const actionRef = useRef<
    ((type: "prev" | "next" | "toggle", x?: number, y?: number) => void) | null
  >(null);
  ...
  const layoutRef = useRef<"default" | "wide" | "scrolled">("scrolled");
  ...
  const [tocTab, setTocTab] = useState<"contents" | "highlights" | "settings">(
    "contents",
  );
  const highlightToastTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (highlightToastTimerRef.current)
        clearTimeout(highlightToastTimerRef.current);
  ...
  useEffect(() => {
    ...
    const nextTheme =
      saved && ["light", "dark", "sepia", "sage", "ink"].includes(saved)
        ? saved
        : "light";
    ...
    const savedLayout = localStorage.getItem("reader-layout") as
      | "default"
      | "wide"
      | "scrolled"
      | null;
    if (
      ...
      (savedLayout === "default" ||
        savedLayout === "wide" ||
        savedLayout === "scrolled")
    ) {
      setLayout(savedLayout);
    } else {
      // Default to scrolled on first load
      setLayout("scrolled");
      localStorage.setItem("reader-layout", "scrolled");
    }
  }, []);

  // Keep root attributes in sync with reader state
  useEffect(() => {
  ...
  useEffect(() => {
    if (bookId) {
      const load = () =>
        getHighlightsByBook(bookId).then((h) => {
          setHighlights(h);
      ...
      channel.onmessage = (event) => {
        if (
          event.data.type === "REFRESH_HIGHLIGHTS" &&
  ...
  const SPREAD_BREAKPOINT = 750;

  const applyLayout = (l: "default" | "wide" | "scrolled") => {
    setLayout(l);
    localStorage.setItem("reader-layout", l);
    const viewer = viewerRef.current?.querySelector(
      "foliate-view",
    ) as FoliateView | null;
    ...
    if (renderer?.setAttribute) {
      if (l === "scrolled") {
        ...
        renderer.setAttribute("max-inline-size", "1440px");
      } else {
        ...
        if (l === "wide") {
          ...
          const useSpread = window.innerWidth >= SPREAD_BREAKPOINT;
          renderer.setAttribute("spread", useSpread ? "both" : "none");
          renderer.setAttribute("max-column-count", useSpread ? "2" : "1");
          renderer.setAttribute(
            "max-inline-size",
            useSpread ? "720px" : "1440px",
          );
        }
      }
    }
  ...
  const triggerAction = useCallback(
    (type: "prev" | "next" | "toggle", x?: number, y?: number) => {
      if (!file) return;
      const viewer = viewerRef.current?.querySelector(
        "foliate-view",
      ) as FoliateView | null;
  ...
  useEffect(() => {
    ...
    if (renderer && renderer.setStyles) {
      if (renderer.setAttribute) {
        const useSpread =
          layout === "default" && window.innerWidth >= SPREAD_BREAKPOINT;
        ...
        if (layout === "scrolled") {
          ...
          renderer.setAttribute(
            "max-column-count",
            layout === "wide" ? "1" : useSpread ? "2" : "1",
          );
          renderer.setAttribute(
            "max-inline-size",
            layout === "wide" ? "9999px" : useSpread ? "720px" : "1440px",
          );
          renderer.setAttribute(
            "spread",
            layout === "wide" ? "none" : useSpread ? "both" : "none",
          );
        }
      ...
      renderer.setStyles(`
        ...
        html, body { 
          ...
          padding: 60px ${layout === "wide" ? "30px" : "40px"} !important;
  ...
  // Re-evaluate spread on window resize (only matters for "default" layout)
  useEffect(() => {
    if (!file || !viewerRef.current) return;
    if (layout === "wide") return; // wide never uses spread
    ...
    const handleResize = () => {
      ...
      renderer.setAttribute("spread", useSpread ? "both" : "none");
      renderer.setAttribute("max-column-count", useSpread ? "2" : "1");
      renderer.setAttribute("max-inline-size", useSpread ? "720px" : "1440px");
    };
  ...
  useEffect(() => {
    ...
    async function initReader() {
      try {
        ...
        // Consolidate dynamic imports to avoid chunk loading issues
        const [
          { Overlayer },
          { makeBook },
          pdfAdapter
          ...
          isPDF ? import("../lib/pdf-adapter") : Promise.resolve(null)
        ]);

        interface SharedBook {
          metadata?: { title?: unknown; author?: unknown };
          toc?: { label: string; href: string }[];
          sections?: { id: string; index: number; cfi?: string }[];
        }

        let bookData: SharedBook;
        if (isPDF && pdfAdapter) {
          bookData = (await pdfAdapter.makePDF(file!)) as unknown as SharedBook;
        } else {
          bookData = (await makeBook(file!)) as unknown as SharedBook;
        }
        if (cancelled) return;
        ...
        const toStr = (val: unknown): string | undefined => {
          ...
          if (Array.isArray(val))
            return val.map(toStr).filter(Boolean).join(", ");
          if (typeof val === "object") {
            // If it has a 'name' key (common for author objects), use that
            if ("name" in val)
              return toStr((val as Record<string, unknown>).name);
            // Otherwise grab the first value (locale key like en-US)
            const keys = Object.keys(val);
            if (keys.length > 0)
              return toStr((val as Record<string, unknown>)[keys[0]]);
          }
          return String(val);
        };
        ...
        viewer.addEventListener("create-overlay", ((e: Event) => {
          ...
          highlightsRef.current.forEach((h) => {
            if (
              h.cfi &&
              (h.cfi.startsWith(prefix + "!") || h.cfi === baseCFI)
            ) {
        ...
        viewer.addEventListener("load", ((e: Event) => {
          ...
          console.log("[Reader] load event fired for section index:", index, "doc:", doc?.title || "(untitled)");
          ...
          if (oldAC) {
            console.log("[Reader] Aborting previous listeners for doc at index:", index);
            oldAC.abort();
          }
          ...
          doc.addEventListener(
            "keydown",
            (ev: KeyboardEvent) => handleKeyDownRef.current(ev),
            { signal },
          );
          ...
          let pointerDownStart: { x: number; y: number; time: number } | null = null;
          
          doc.addEventListener("pointerdown", (ev: PointerEvent) => {
            console.log("[Reader] pointerdown on doc index:", index, "type:", ev.pointerType, "x:", ev.clientX, "y:", ev.clientY);
            pointerDownStart = { x: ev.clientX, y: ev.clientY, time: Date.now() };
          }, { signal });

          // Navigation handling via pointer events to bypass touchmove + preventDefault suppression from foliate-js
          doc.addEventListener(
            "pointerup",
            (ev: PointerEvent) => {
              console.log("[Reader] pointerup on doc index:", index, "type:", ev.pointerType, "defaultPrevented:", ev.defaultPrevented, "x:", ev.clientX, "y:", ev.clientY);
              if (ev.defaultPrevented) return;
              
              // Only process actual taps (little movement, quick duration)
              if (pointerDownStart) {
                const dx = Math.abs(ev.clientX - pointerDownStart.x);
                const dy = Math.abs(ev.clientY - pointerDownStart.y);
                const dt = Date.now() - pointerDownStart.time;
                pointerDownStart = null;
                
                console.log("[Reader] tap guard: dx=", dx, "dy=", dy, "dt=", dt);
                // Allow a small threshold for finger roll, and max 500ms for a tap
                if (dx > 15 || dy > 15 || dt > 500) {
                  console.log("[Reader] REJECTED: swipe/longpress");
                  return; // This was a swipe or a long press
                }
                console.log("[Reader] ACCEPTED: tap");
              } else {
                console.log("[Reader] WARNING: no pointerDownStart recorded");
              }
              ...
              if (
                highlightToolbarRef.current ||
                pendingNoteHighlightRef.current ||
                hasSelection
              ) {
                if (highlightToolbarRef.current) setHighlightToolbar(null);
                if (pendingNoteHighlightRef.current)
        ...
        const oldViewer = container.querySelector(
          "foliate-view",
        ) as FoliateView | null;
        if (oldViewer) {
          try {
        ...
        if (renderer) {
          const useSpread =
            ...
            window.innerWidth >= SPREAD_BREAKPOINT;

          if (layoutRef.current === "scrolled") {
            renderer.setAttribute("flow", "scrolled");
            renderer.setAttribute("spread", "none");
            renderer.setAttribute("max-column-count", "1");
            renderer.setAttribute("max-inline-size", "1440px");
            ...
            renderer.setAttribute(
              "max-column-count",
              layoutRef.current === "wide" ? "1" : useSpread ? "2" : "1",
            );
            renderer.setAttribute(
              "max-inline-size",
              layoutRef.current === "wide"
                ? "9999px"
                : useSpread
                  ? "720px"
                  : "1440px",
          ...
          if (renderer.setStyles) {
            renderer.setStyles(`
                 ...
                 html, body { 
                   ...
                   max-width: 100% !important;
                   margin: 0 auto !important;
                   padding: 60px ${layoutRef.current === "wide" ? "30px" : "40px"} !important;
                   box-sizing: border-box !important;
                   touch-action: manipulation !important;
                   user-select: none !important;
                   -webkit-user-select: none !important;
    ...
    return () => {
      ...
      if (viewer) {
        // foliate-js cleanup can sometimes trigger ResizeObserver errors if the element is removed too quickly
        // We wrap the cleanup in a try-catch and requestAnimationFrame to ensure the DOM is stable
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      // If we're on mobile, the backdrop click handles closing.
      // On desktop, we check if click is outside textMenuRef.
      if (
        isMenuOpen &&
  ...
  }, [isMenuOpen, pendingNoteHighlight]);
  ...
  return (
    <div className="readerContainer">
      ...
      >
        ...
        <header className={`header ${(!showHeader && !isMenuOpen) ? "header-hidden" : ""}`}>
        ...
        <div className="tocOverlay" onClick={() => setIsMenuOpen(false)}>
          ...
          >
            ...
            <div className="tocTabs">
              ...
              <button
                className={`tocTabBtn ${tocTab === "settings" ? "active" : ""}`}
                onClick={() => setTocTab("settings")}
              >
                Settings
              </button>
            </div>
            ...
            {tocTab === "contents" ? (
                  ...
                  highlights
                    ...
                    .map((h) => (
                      <div
                        ...
                        onClick={() => {
                          const v = viewerRef.current?.querySelector(
                            "foliate-view",
                          ) as FoliateView | null;
                          if (v && h.cfi) {
                            // Capture current position if not already in preview mode
                            if (
                              originalCfiRef.current === null &&
                              lastCfiRef.current
                            ) {
                              updateOriginalCfi(lastCfiRef.current);
                              originalFractionRef.current =
                      ...
                      >
                        <div className="highlightItem-header">
                          <span
                            className="highlightItem-chapter"
                            style={{ color: h.color }}
                          >
              ...
              >
                      ...
                      style={{
                        ...
                        color:
                          layout === "scrolled"
                            ? "var(--accent-color)"
                            : "inherit",
                        border: `1px solid ${layout === "scrolled" ? "var(--accent-color)" : "var(--glass-border)"}`,
                      }}
                    >
                      Scrolled
                    </button>
                      ...
                      style={{
                        ...
                        color:
                          layout === "wide"
                            ? "var(--accent-color)"
                            : "inherit",
                        border: `1px solid ${layout === "wide" ? "var(--accent-color)" : "var(--glass-border)"}`,
                      }}
                    >
                      Wide
                    </button>
                    {windowWidth >= SPREAD_BREAKPOINT && (
                      <button
                        className="btn flex-1 text-center py-2"
                        ...
                        style={{
                          ...
                          color:
                            layout === "default"
                              ? "var(--accent-color)"
                              : "inherit",
                          border: `1px solid ${layout === "default" ? "var(--accent-color)" : "var(--glass-border)"}`,
                        }}
                      >
                        Default
                      </button>
                ...
                <div>
                  ...
                  >
                    {(["light", "dark", "sepia", "sage", "ink"] as const).map(
                      (t) => (
                        <button
                          key={t}
                          ...
                          style={{
                            ...
                            background:
                              theme === t
                                ...
                                : "transparent",
                            color:
                              theme === t ? "var(--accent-color)" : "inherit",
                            border: `1px solid ${theme === t ? "var(--accent-color)" : "var(--glass-border)"}`,
                          }}
                        >
                          <span
                            aria-hidden="true"
      ...
      <div
        ref={viewerRef}
        className={`viewer ${isAnimating ? "slide-in-right" : ""}`}
      />
            ...
            >
              <span
                style={{
                  ...
                  height: 14,
                  borderRadius: "50%",
                  background: noteModal.color,
                  flexShrink: 0,
                }}
              />
              <strong style={{ fontSize: "0.95rem" }}>
                {noteModal.id ? "Edit Note" : "Add a Note"}
              </strong>
            </div>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  setHighlights((prev) =>
                    prev.map((h) =>
                      h.id === targetId
                        ? { ...h, note: noteInput.trim() || undefined }
                        : h,
                    ),
                  );
      ...
      {/* Highlight Toast */}
      {highlightToast && (
        <div className="highlight-toast">✨ Highlight saved</div>
      )}
