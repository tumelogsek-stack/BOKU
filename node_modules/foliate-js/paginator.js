...
const getVisibleRange = (doc, start, end, mapRect) => {
    ...
    if (!root) return doc.createRange()
    const walker = doc.createTreeWalker(root, filter, { acceptNode })
    const nodes = []
    for (let node = walker.nextNode(); node; node = walker.nextNode())
        nodes.push(node)

    // we're only interested in the first and last visible nodes
    const from = nodes[0] ?? root
...
class View {
    ...
    render(layout) {
        if (!layout || !this.document || !this.document.body) return
...
export class Paginator extends HTMLElement {
    ...
    #justAnchored = false
    // #locked = false // while true, prevent any further navigation (disabled)
    #styles
    ...
    scrollBy(dx, dy) {
        if (!this.#view || !this.#scrollBounds) return
    ...
    #canGoToIndex(index) {
        return this.sections && index >= 0 && index <= this.sections.length - 1
    }
    ...
    setStyles(styles) {
        ...
        requestAnimationFrame(() => {
            if (view === this.#view) this.#replaceBackground(view.docBackground, this.columnCount)
        })
        
        // needed because the resize observer doesn't work in Firefox
        view.document?.fonts?.ready?.then(() => {
            if (view === this.#view) view.expand()
        })
    }
    focusView() {
